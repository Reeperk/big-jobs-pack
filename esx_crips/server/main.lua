...
RegisterNetEvent('esx_aztecasjob:confiscatePlayerItem')
AddEventHandler('esx_aztecasjob:confiscatePlayerItem', function(target, itemType, itemName, amount)
	...
	if sourceXPlayer.job.name ~= 'aztecas' then
		print(('esx_aztecasjob: %s attempted to confiscate!'):format(xPlayer.identifier))
		return
	end
...
AddEventHandler('esx_aztecasjob:handcuff', function(target)
	...
	if xPlayer.job.name == 'aztecas' then
		...
		print(('esx_aztecasjob: %s attempted to handcuff a player (not cop)!'):format(xPlayer.identifier))
	end
end)
...
AddEventHandler('esx_aztecasjob:drag', function(target)
	...
	if xPlayer.job.name == 'aztecas' then
		TriggerClientEvent('esx_aztecasjob:drag', target, source)
	else
		print(('esx_aztecasjob: %s attempted to drag (not cop)!'):format(xPlayer.identifier))
	end
end)
...
RegisterNetEvent('esx_aztecasjob:getStockItem')
...
RegisterNetEvent('esx_aztecasjob:putStockItems')
...
ESX.RegisterServerCallback('esx_aztecasjob:getOtherPlayerData', function(source, cb, target, notify)
...
ESX.RegisterServerCallback('esx_aztecasjob:buyWeapon', function(source, cb, weaponName, type, componentNum)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if not selectedWeapon then
		print(('esx_aztecasjob: %s attempted to buy an invalid weapon.'):format(xPlayer.identifier))
		cb(false)
	else
		-- Weapon
		if type == 1 then
		...
		elseif type == 2 then
			...
			if component then
				...
				print(('esx_aztecasjob: %s attempted to buy an invalid weapon component.'):format(xPlayer.identifier))
				cb(false)
			end
		end
	end
...
ESX.RegisterServerCallback('esx_aztecasjob:buyJobVehicle', function(source, cb, vehicleProps, type)
	local xPlayer = ESX.GetPlayerFromId(source)
	local price = getPriceFromHash(vehicleProps.model, xPlayer.job.grade_name, type)

	-- vehicle model not found
	if price == 0 then
		print(('esx_aztecasjob: %s attempted to exploit the shop! (invalid vehicle model)'):format(xPlayer.identifier))
		cb(false)
	else
		if xPlayer.getMoney() >= price then
			xPlayer.removeMoney(price)
...
ESX.RegisterServerCallback('esx_aztecasjob:storeNearbyVehicle', function(source, cb, nearbyVehicles)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if not foundPlate then
		...
		MySQL.Async.execute('UPDATE owned_vehicles SET `stored` = true WHERE owner = @owner AND plate = @plate AND job = @job', {
			...
			if rowsChanged == 0 then
				print(('esx_aztecasjob: %s has exploited the garage!'):format(xPlayer.identifier))
				cb(false)
			else
				cb(true, foundNum)
			end
...
RegisterNetEvent('esx_aztecasjob:spawned')
AddEventHandler('esx_aztecasjob:spawned', function()
	local xPlayer = ESX.GetPlayerFromId(playerId)
...
RegisterNetEvent('esx_aztecasjob:forceBlip')
